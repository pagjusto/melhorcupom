...
export const CouponDetailModal = ({ coupon, store, onClose, onTestValidateAtMerchant }) => {
  const { redeemCoupon, redemptions, isVipUser, setIsSubscriptionModalOpen } = useApp();
  ...
  useEffect(() => {
    ...
    // Verificar se já existe resgate válido para este cupom
    const existing = redemptions.find(r => r.couponId === coupon.id && r.status === 'valid');
    if (existing) {
  ...
  if (!coupon || !activeRedemption) return null;
